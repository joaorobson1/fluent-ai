...
export function TaskMultipleChoice({ content, onSubmit }: Props) {
  ...
  // O correct_index vem do banco (via API) — é a fonte de verdade.
  // Fallback para o content local apenas se a API não retornar.
  const authorativeCorrectIndex = result?.correct_index ?? content.correct_index;
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {content.options.map((option, i) => {
          ...
          // Estado visual deriva 100% do resultado da API
          ...
          if (result) {
            if (result.correct && isSelected) {
              ...
              state = "wrong";
            } else if (!result.correct && i === authorativeCorrectIndex) {
              state = "correct-reveal"; // revelar a resposta certa
            }
          }
